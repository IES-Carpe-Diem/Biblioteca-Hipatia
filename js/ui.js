...
//   - NUEVO: Modal paginado de portadas (20 por página)
...
function updateFontSize(value) {
    ...
    if (fontValueSpan) fontValueSpan.textContent = numValue + '%';
}
...
function updateLetterSpacing(value) {
    ...
    if (letterValueSpan) letterValueSpan.textContent = numValue + '%';
}
...
function updateButtonSize(value) {
    ...
    if (buttonValueSpan) buttonValueSpan.textContent = numValue + '%';
}
...
function updateContrast(value) {
    ...
    if (contrastValueSpan) contrastValueSpan.textContent = numValue + '%';
}
...
function updateBrightness(value) {
    ...
    if (brightnessValueSpan) brightnessValueSpan.textContent = numValue + '%';
}
    ...
    content.innerHTML = `
        ...
        <div class="accessibility-section">
            ...
            <div class="accessibility-group">
                ...
                <input type="range" id="contrast-slider" min="0" max="100" value="50" step="1" oninput="updateContrast(this.value)">
            </div>
            <div class="accessibility-group">
                <label for="brightness-slider">Brillo (<span id="brightness-value">50%</span>)</label>
                <input type="range" id="brightness-slider" min="0" max="100" value="50" step="1" oninput="updateBrightness(this.value)">
            </div>
            <div class="accessibility-group">
                <label>Daltonismo</label>
                <div class="daltonism-options">
                    <div class="daltonism-option">
                        <input type="radio" id="daltonism-normal" name="daltonism" value="normal" ${currentDaltonism === 'normal' ? 'checked' : ''}>
                        <label for="daltonism-normal">Normal</label>
                    </div>
                    <div class="daltonism-option">
                        <input type="radio" id="daltonism-deuteranomaly" name="daltonism" value="deuteranomaly" ${currentDaltonism === 'deuteranomaly' ? 'checked' : ''}>
                        <label for="daltonism-deuteranomaly">Deuteranomalía (verde-rojo)</label>
                    </div>
                    <div class="daltonism-option">
                        <input type="radio" id="daltonism-protanomaly" name="daltonism" value="protanomaly" ${currentDaltonism === 'protanomaly' ? 'checked' : ''}>
                        <label for="daltonism-protanomaly">Protanomalía (rojo-verde)</label>
                    </div>
                    <div class="daltonism-option">
                        <input type="radio" id="daltonism-tritanomaly" name="daltonism" value="tritanomaly" ${currentDaltonism === 'tritanomaly' ? 'checked' : ''}>
        ...
        <div class="accessibility-section">
            ...
            <div class="accessibility-group">
                ...
                <input type="range" id="font-size-slider" min="0" max="100" value="50" step="1" oninput="updateFontSize(this.value)">
            </div>
            <div class="accessibility-group">
                <label for="letter-spacing-slider">Espacio entre Letras (<span id="letter-spacing-value">50%</span>)</label>
                <input type="range" id="letter-spacing-slider" min="0" max="100" value="50" step="1" oninput="updateLetterSpacing(this.value)">
            </div>
            <div class="accessibility-group">
                <label for="button-size-slider">Tamaño de Botones (<span id="button-size-value">50%</span>)</label>
                <input type="range" id="button-size-slider" min="0" max="100" value="50" step="1" oninput="updateButtonSize(this.value)">
            </div>
        </div>
        <div class="accessibility-section">
...
function mostrarResultadosModal() {
    ...
    initEstrellas();
    // iniciarRotativas();  // Comentado para evitar rotación automática
}
...
function abrirModalPortadas() {
    const modal = document.getElementById('portadasModal');
    const grid = document.getElementById('portadasGrid');
    const pagination = document.getElementById('portadasPagination');

    const conPortadas = librosConPortadas.filter(l => l.portadas.length > 0);
    if (conPortadas.length === 0) {
        grid.innerHTML = '<p style="text-align:center; padding:20px;">No hay portadas disponibles en este momento.</p>';
        pagination.style.display = 'none';
    } else {
        const PAGE_SIZE = 20;
        const totalPages = Math.ceil(conPortadas.length / PAGE_SIZE);
        renderPortadasModalGrid(grid, conPortadas, 1, PAGE_SIZE);
        renderPagination('portadasPagination', 1, totalPages, (page) => renderPortadasModalGrid(grid, conPortadas, page, PAGE_SIZE));
    }

    modal.style.display = 'block';
    setupFocusTrap('portadasModal');
    document.querySelector('.theme-controls').classList.add('active');
}

function cerrarModalPortadas() {
    cleanupFocusTrap('portadasModal');
    document.getElementById('portadasModal').style.display = 'none';
    document.querySelector('.theme-controls').classList.remove('active');
}

function renderPortadasModalGrid(grid, conPortadas, currentPage, pageSize) {
    const start = (currentPage - 1) * pageSize;
    const end = start + pageSize;
    const paginated = conPortadas.slice(start, end);

    grid.innerHTML = paginated.map(libro => `
        <div style="text-align:center; cursor:pointer;" onclick="openSearchForBook('${libro.id}'); cerrarModalPortadas();">
            <img src="${libro.portadas[0]}" alt="Portada de ${libro.titulo}"
                 loading="lazy" onerror="this.src='${CONFIG.DEFAULT_PORTADA}'"
                 style="width:100%; max-height:200px; object-fit:cover; border-radius:8px; box-shadow:var(--shadow); transition: transform 0.2s;">
            <p style="margin:8px 0; font-size:0.9em; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                ${libro.titulo}
            </p>
        </div>
    `).join('');
