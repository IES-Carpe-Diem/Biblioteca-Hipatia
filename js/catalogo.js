...
//   - Preparación de datos para el modal paginado de portadas (20 por página)
//   - Renderizado de portadas individuales en modales y grid
//   - Portadas rotativas DESACTIVADAS (solo muestra primera imagen estática)
...
async function cargarPortadasDesdeGitHub() {
    try {
        ...
        renderRecomendados();
        // iniciarRotativas();  // COMENTADO: desactivamos la rotación automática
...
// Función de rotativas DESACTIVADA (solo se muestra la primera imagen estática si se llama)
function iniciarRotativas() {
    const conPortadas = librosConPortadas.filter(l => l.portadas.length > 0);
    if (conPortadas.length === 0) {
        document.getElementById('rotativasContainer').innerHTML = '<p style="text-align: center; color: var(--text-secondary);">No hay portadas disponibles.</p>';
        return;
    }
    const container = document.getElementById('rotativasContainer');
    container.innerHTML = '';
    // Solo mostramos la primera portada estática
    const libro = conPortadas[0];
    const img = document.createElement('img');
    img.src = libro.portadas[0];
    img.alt = `Portada de ${libro.titulo}`;
    img.setAttribute('aria-label', `Portada destacada: ${libro.titulo}`);
