...
//   - Recomendaciones diarias (con fallback a portada default si no hay portadas)
//   - Renderizado de portadas en modales y grid

let catalogo = [];
let librosConPortadas = [];

// Función auxiliar para actualizar librosConPortadas usando las portadas del catálogo (cached o nuevas)
...
async function cargarPortadasDesdeGitHub() {
    try {
        ...
        console.log('Portadas actualizadas desde GitHub');
        renderRecomendados();
        precargarPortadasClave();

    } catch (err) {
        console.error('Error cargando portadas nuevas (GitHub)', err);
        actualizarLibrosConPortadas(); // Usa siempre lo que haya en cache
        renderRecomendados();
    }
}
...
function renderRecomendados() {
    ...
    // Siempre muestra 5 libros (fallback a catálogo completo si no hay con portadas)
    let seleccionados = [];
    ...
    if (seleccionados.length < 5 || seleccionados.length === 0) {
        ...
        const shuffled = [...catalogo].sort(() => seededRandom() - 0.5);
